...
if(isset($_GET['id'])){
  $id=$_GET['id'];
$ser=$_GET['ser'];
if($_GET['ser']=="طالب وساطة إشعار مبدئي"){
$price=99; 
}else{
$price=199; 
}}
...
<body>
  ...
  <nav class="navbar navbar-expand  navbar-dark" style="background-color: black; opacity: 100%;">
    <a  style="color:white;float: left; padding: 5px; " ></a>
    <a  style="color:white;float: left; padding: 5px;"></a>
...
<div class="topnav sticky-top " id="myTopnav" >
 ...
 <a href="request.php" style="color: rgb(206, 180, 117)!important;" >طلب وساطة</a>
...
<nav class="w3-sidebar w3-bar-block w3-card w3-animate-left w3-hide-medium w3-hide-large" style="display:none ; z-index: 99;" id="mySidebar">
  ...
  <a href="Ramakeen.html" onclick="w3_close()" class=" w3-button nav-link"   ><span class="menuside">الرئيسية</span></a><hr>
  ...
  <a href="contact.php" onclick="w3_close()" class=" w3-button nav-link"> <span class="menuside"> اتصل بنا </span></a> <hr>
    ...
    <!-- Table Reservation Form -->
        ...
        if(isset($_GET['pay'])){
          if($_GET['pay']==1){
          echo '<div style=""><div class="alert alert-success"  style="max-width: 600px;"  role="alert"><h4 style="text-align:center;">تم الدفع بنجاح</h4> </div></div>';
        }else{
          echo '<div style=""><div class="alert alert-danger"  style="max-width: 600px;"  role="alert"><h4 style="text-align:center;">لم يتم عمل الدفع بنجاح  </h4> </div></div>';
        }
      }
        ?>
    				<h2 style=" font-weight: 600; letter-spacing: .5px;" > عملية الدفع    </h2>

    			</div>
    		</div> <br> <br>
        <form id="form-container" method="post" action="tap.php?id=<?php echo $id;?>&price=<?php echo $price;?>">
          ...
          <!-- Tap pay button -->
          <button id="tap-btn" class="btn btn-rounded btn-lg btn-block"
           style=" background-color: rgb(206, 180, 117);
            font-size: 22px;
            color: white;" >إرسال</button>
        </form>
